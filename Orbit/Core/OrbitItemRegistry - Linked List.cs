...
namespace Orbit.Core
{
	...
	public class OrbitItemRegistry
	{
		...
		private OrbitItemLinkedList ItemRegistry;
		...
		{
			ItemRegistry=new OrbitItemLinkedList();
			...
			try
			{
				while(this.LineRegistry.Length>line)
				{
					if(LineRegistry[this.LineRegistry.Length-1].TS!=null)
						LineRegistry[this.LineRegistry.Length-1].TS.AbortSync();
		...
		public void CreateRoot(OrbitItem[] items)
		{
			try
			{
				// make sure we're empty
		...
		public void ExpandFrom(OrbitItem parentItem)
		{
			...
			try
			{
				...
				if(parentItem.GetType()==typeof(StoredOrbitItem)
					|| parentItem.GetType()==typeof(FileSystemDirectoryItem))
				{
					// get the path
					if(parentItem.GetType()==typeof(FolderItem))
						Path=((StoredOrbitItem)parentItem).ItemPath;
					if(parentItem.GetType()==typeof(FileSystemDirectoryItem))
						Path=((FileSystemDirectoryItem)parentItem).Path;
				...
				if(Global.Configuration.Appearance.ShowImageThumbnails)
				{
					LineRegistry[LineRegistry.Length-1].TS=new ThumbnailSync(NewItems, this.LineRegistry.Length-1);
				}
			}
			catch(Exception)
			{
		...
		public OrbitItemLinkedList Items
		{
			get
			{
				return ItemRegistry;
